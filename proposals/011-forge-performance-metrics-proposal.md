...
Forge uses tools like Jira, GitHub and LLM. We want to track how long it takes to finish a feature, how much it. If its worth it.
## 2. Motivation
...
An **iteration** is when AI generates something and a human reviews it. We track how iterations it takes to finish a feature.
...
retry_recovery_rate` | Percentage of retries that successfully resume | We want than 90 percent |
...
| `ci_skip_post_merge_regression` | Defects in production to skipped gates | Our target is zero |
...
Metric | Description | Target |
|--------|-------------|--------|
| `pr_desc_sync_count` | Number of description regenerations per PR | We will track this it correlates with CI fix count
| `pr_desc_sync_latency` | Time to regenerate description | We aim for less than 15 seconds |
| `review_time_after_sync` | Whether synced descriptions reduce human review time | We will compare pre. Post |
...
Metric | Description | Target |
|--------|-------------|--------|
| `repo_config_resolution_time` | Time to resolve repository configuration | We aim for than 200 milliseconds |
...
| Shared Workspace | workspace across workers | Reduce context reconstruction overhead |
---
