...
async def update_docs_repo(state: WorkflowState) -> WorkflowState:
    ...
    if not current_repo or "/" not in current_repo:
        logger.warning(f"Invalid current_repo '{current_repo}' for {ticket_key}")
        return state

    logger.info(f"Updating separate docs repo {docs_repo} for {ticket_key}")

    code_owner, code_repo_name = current_repo.split("/", 1)
    code_default_branch = await _get_repo_default_branch(settings, code_owner, code_repo_name)
    ...
    default_branch = await _get_repo_default_branch(settings, owner, repo)
...
async def _get_repo_default_branch(settings: Settings, owner: str, repo_name: str) -> str:
    """Resolve the default branch for a given repository."""
    ...
    try:
        ...
        logger.warning(
            f"Could not fetch repository metadata for {owner}/{repo_name}, defaulting to 'main': {e}"
        )
        return "main"
    finally:
        await github.close()
...
def _branch_has_commits(workspace_path: Path, base_branch: str = "main") -> bool:
    ...
    try:
        ...
        return git.has_commits_ahead(base_branch)
    except Exception as e:
        logger.warning(f"Error checking if branch has commits ahead of origin/{base_branch}: {e}")
        return False
