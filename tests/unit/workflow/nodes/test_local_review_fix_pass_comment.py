...
class TestLocalReviewFixPassCommentErrorHandling:
    """Test error handling for fix pass comment posting."""

    @pytest.mark.asyncio
    async def test_workflow_continues_when_fix_pass_comment_fails(self, caplog):
        """Should continue workflow when fix pass comment posting fails."""
        mock_jira = create_mock_jira_client()
        mock_runner = create_mock_container_runner(has_unfixed_issues=False)
        mock_git = create_mock_git_operations(has_changes=False)
        ...
        # Mock post_status_comment to raise an exception
        async def failing_post_status_comment(*args, **kwargs):
        ...
        with (
            ...
            patch(
                "forge.workflow.nodes.local_reviewer.post_status_comment",
                side_effect=failing_post_status_comment,
            ),
        ):
        ...
        # Verify JiraClient was still properly closed
        mock_jira.close.assert_called_once()

    @pytest.mark.asyncio
    async def test_jira_client_closed_even_on_exception(self):
        """Should close JiraClient even if post_status_comment raises exception."""
        mock_jira = create_mock_jira_client()
        mock_runner = create_mock_container_runner(has_unfixed_issues=False)
        mock_git = create_mock_git_operations(has_changes=False)
        ...
        # Verify JiraClient.close() was called despite exception
        mock_jira.close.assert_called_once()
