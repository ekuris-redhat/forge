...
class TestTriageCheckMissingFields:
    ...
    @pytest.mark.asyncio
    async def test_triage_attempts_incremented(
        self, incomplete_ticket_state, mock_jira, mock_agent_missing_fields
    ):
        """triage_attempts is incremented by 1."""
        from forge.workflow.nodes.triage import triage_check

        assert incomplete_ticket_state.get("triage_attempts", 0) == 0
        with (
            patch("forge.workflow.nodes.triage.JiraClient", return_value=mock_jira),
            patch(
                "forge.workflow.nodes.triage.ForgeAgent",
    ...
    @pytest.mark.asyncio
    async def test_targeted_comment_lists_exact_missing_fields(
        self, incomplete_ticket_state, mock_jira, mock_agent_missing_fields
    ):
        """Targeted comment lists only the exact missing fields."""
        from forge.workflow.nodes.triage import triage_check
        ...
        with (
            ...
            await triage_check(incomplete_ticket_state)
        # Check comment contains exact missing fields
        last_comment = mock_jira.add_comment.call_args_list[-1].args[1]
        assert "- steps_to_reproduce" in last_comment
        assert "- environment" in last_comment
        assert "- affected_versions" not in last_comment
